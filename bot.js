...
const { getLevelLossRule, getNextLevelAfterLoss } = require('./level-rules');
const { detectCondition3 } = require('./pattern-rules');
const { getMartingaleBetAmount, getBetSequence } = require('./bet-amounts');

// ============================================================
//  CONFIG
// ============================================================
const BOT_TOKEN    = process.env.BOT_TOKEN || "8999335291:AAF1KCxFkVsnyBWpdOIdmFeBV9UnhXBOa6U";
...
// Martingale multipliers — user can customize exact custom bet sequence
...
function initState(userId) {
    if (!userStates[userId]) {
        ...
        if (userStates[userId].predCurL === undefined) userStates[userId].predCurL = 0;
    }
}
...
function decidePrediction(list, currentLevel, userId) {
    ...
    // Priority 2: Condition 3 streak trigger
    const condition3Signal = detectCondition3(list);
    if (condition3Signal) {
        return condition3Signal;
    }

    // Priority 3: Condition 2 immediate-bet trigger
    if (list.length >= 4) {
        ...
        if (alt1 || alt2) {
            ...
            return { type: "SIZE", val: prediction, conf: 92, pat: "COND2", action: { opposite: true, immediateBet: true, afterLossSkip: 3 } };
        }
    }

    // Priority 4: Condition 4 — 6 consecutive prediction losses triggers immediate recovery bet at current level
    if (state && state.predCurL >= 6) {
        const last = buildBSFromList(list, 1)[0] || "BIG";
        ...
        return {
            ...
            conf: 100,
            pat: "COND4",
            action: { immediateBet: true, afterLossSkip: 3, overrideSkip: true }
        };
    }
...
function getPredictionReason(signal) {
    if (!signal) return "No reason available";
    const pat = signal.pat || "UNKNOWN";
    switch (pat) {
        case "COND1": return "Sequence of 6 triggers skip";
        case "COND2": return "Alternating pattern triggers immediate bet";
        case "COND2_CONTINUE": return "Continuing opposite-mode bet";
        case "COND3": return "Repeated number skip";
        case "COND4": return "6 consecutive losses triggers recovery bet";
        case "LAST5": return signal.val === "SKIP" ? "Tie in last 5 results" : "Majority of last 5 results";
        default: return signal.action && signal.action.immediateBet ? "Immediate bet condition" : `Pattern: ${pat}`;
    }
}
...
function updateAfterResult(userId, wasWin, actual, betPlaced, betAmount = null) {
    ...
    if (st) {
        ...
        if (wasWin) {
            ...
            state.predCurL = (state.predCurL || 0) + 1;
        ...
        if (betPlaced) {
            ...
            if (state.conditionImmediate) {
                const afterLossSkip = Number(state.conditionImmediate.afterLossSkip) || 3;
                if (wasWin) {
                    st.level = 1;
                    ...
                    const immLevel = Number(state.conditionImmediate && state.conditionImmediate.immediateLevel) || Number(st.level) || 1;
                    const maxLvl = (cfg && Number(cfg.maxLvl)) ? Number(cfg.maxLvl) : immLevel;
                    const nextLvl = getNextLevelAfterLoss(immLevel, maxLvl);
                    st.level = nextLvl;
                    state.postSkipSetLevel = nextLvl;
                }
                delete state.conditionImmediate;
            } else {
                if (wasWin) {
                    st.level = 1;
                    st.consecutiveLoss = 0;
                    ...
                    const rule = getLevelLossRule(lostLevel);
                    const nextLevel = getNextLevelAfterLoss(lostLevel, Number(cfg.maxLvl) || 15);
                    if (rule.type === 'skip') {
                        state.isSkipping = true;
                        state.skipCount = 1;
                        ...
                        state.levelWatchTarget = nextLevel;
...
async function runPredict(userId, chatId) {
    ...
    // If a condition requested an immediate bet, cancel current skip and mark immediate flag
    try {
        if (signal && signal.action && signal.action.immediateBet && (!state.isSkipping && !state.skipWatch || signal.action.overrideSkip)) {
            ...
            state.conditionImmediate = {
                afterLossSkip: Number(signal.action.afterLossSkip) || 3,
                immediateLevel: explicitLvl || currentLvl,
                overrideSkip: Boolean(signal.action.overrideSkip)
            };
            await send(chatId, `⚡ ${signal.pat || 'Immediate condition'} triggered — placing an immediate bet at L${state.conditionImmediate.immediateLevel}.`);
        }
    } catch (e) {
        console.error('[IMMEDIATE CONDITION ERR]', e && e.message);
    }
    ...
    try {
        if (signal && signal.action && signal.action.skip && !state.isSkipping && !state.skipWatch) {
            ...
            if (Number(st.level) > 3) {
                ...
                state.skipWatch = true;
                await send(chatId, `⏳ ${signal.pat || 'Condition skip'} matched — skipping ${state.skipTotal} predictions.`);
                return setTimeout(() => { if (running[userId]) runPredict(userId, chatId); }, 3000);
            }
            await send(chatId, `⏭️ ${signal.pat || 'Condition skip'} matched but skip is blocked before L4.`);
            return setTimeout(() => { if (running[userId]) runPredict(userId, chatId); }, 3000);
        }
    } catch (e) {
    ...
    let canBet = false;
    const signalName = signal.val === "SKIP" ? "⏭️ SKIP" : signal.val === "BIG" ? "🔵 BIG" : "🟠 SMALL";
    const patternName = signal && signal.pat ? signal.pat : (state && state.mode ? state.mode : "NORMAL");
    const reasonText = getPredictionReason(signal);
...
"╚══════════════════════════╝"
        ...
        // Continue to result tracking even during skip/watch periods.
...
"╚══════════════════════════╝",
    ...
    if (canBet) {
        levelToUse = (state.conditionImmediate && state.conditionImmediate.immediateLevel) || (signal.action && signal.action.immediateLevel) || st.level;
        ...
        if (result && result.ok) {
            ...
            const failMessage = result && !result.ok ? (result.msg || "Unknown error") : "Bet was not placed.";
            await send(chatId, "❌ Bet Failed: " + failMessage + "\n⏳ Result will still be monitored for prediction accuracy.");
        }
    }

    // Track the result even when no bet was placed (skip/watch/no-bet periods still count toward predCurL).
...
async function levelAnalysisReport(chatId, userId, maxShow = 15) {
    initUser(userId);
    const pt = profitTrack[userId] || {};
    const la = buildLevelAnalysis(pt, maxShow);
    if (!la.lines.length) {
        return send(chatId, `📊 L1-L${maxShow} Analysis:\nNo placed bets yet.`);
    }

    let report = `📊 L1-L${maxShow} Analysis\n\n`;
    report += la.lines.join("\n") + "\n\n";
    report += la.summary;
    send(chatId, report);
}
...
"/setmytoken TOKEN",
        ...
        if(text==="L1-L15" || text==="l1-l15" || text==="l1 l15") return await levelAnalysisReport(msg.chat.id, id, 15);
...
"🚀 ENGINE ON!\n\nAutoBet: "+(cfg.enabled?"✅ ON":"❌ OFF")+"\nWatch  : "+(cfg.watch?"ON ("+cfg.watchLoss+"L)":"OFF")+"\nCustom : ₹"+(cfg.customBets||[]).join("→₹")+" | MaxLvl: "+cfg.maxLvl
            );
